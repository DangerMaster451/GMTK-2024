...
class Block():
    ...
    def renderInventory(self, rel_x, rel_y) -> None:
        print(self.surface)
        for coord in self.structure:
            self.surface.blit(self.image, (self.scale_factor * coord[0] * self.pixel_size + rel_x, self.scale_factor * coord[1] * self.pixel_size + rel_y))
...
class Single(Block):
    def __init__(self, surface, pixel_size, scale_factor) -> None:
