...
class Inventory(pygame.Surface):
    def __init__(self, size_x, size_y, blocks):
        ...
        self.scale_size = self.size_x/5/self.size_x
    ...
    def render(self):
        ...
        for x in range(0, min(len(self.blocks), self.block_count)):
            if self.blocks[x] == 0:
                s = Single(self, 128, 0.2)
                s.renderInventory(x*216, 0)
            elif self.blocks[x] == 1:
                s = Short_Line_H(self, 128, 0.2)
                s.renderInventory(x*216, 0)
            elif self.blocks[x] == 2:
                s = Long_Line_H(self, 128, 0.2)
                s.renderInventory(x*216, 0)
            elif self.blocks[x] == 3:
                s = Short_Line_V(self, 128, 0.2)
                s.renderInventory(x*216, 0)
            elif self.blocks[x] == 4:
                s = Long_Line_V(self, 128, 0.2)
                s.renderInventory(x*216, 0)